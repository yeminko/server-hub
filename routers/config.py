...
def create_config(
    ...
    path: str = Header(...,
                       description="Path to group configs, e.g. 'pos-app/local/frontend'"),
    ...
    for key, value in configs.items():
        ...
        db_config = db.query(Config).filter(
            Config.path == path, Config.key == key).first()
...
def read_configs_by_path(
    ...
    if not configs:
        raise HTTPException(
            status_code=404, detail=f"No configs found for path: {path}")
...
def read_config_by_key(
    ...
    if not db_config:
        raise HTTPException(
            status_code=404, detail=f"Key '{key}' not found in path '{path}'")
...
def update_configs(
    ...
    if not updated_keys:
        raise HTTPException(
            status_code=404, detail=f"No matching configs found in path '{path}'")
